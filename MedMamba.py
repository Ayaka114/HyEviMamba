...
class RawImageHyperADExtractor(nn.Module):
    ...
    def forward(self, x):
        ...
        W_down = down_params[:, :self.reduced_dim*self.feature_dim].reshape(B, self.reduced_dim, self.feature_dim)  # B,R,F
        ...
        W_up = up_params[:, :self.feature_dim*self.reduced_dim].reshape(B, self.feature_dim, self.reduced_dim)  # B,F,R
...
class VSSM(nn.Module):
    def __init__(self, patch_size=4, in_chans=3, num_classes=1000, depths=[2, 2, 4, 2], depths_decoder=[2, 9, 2, 2],
                 ...
                 use_checkpoint=False, hyper_ad=False,EDL=False,reduction_ratio=4 , raw_hyper_ad_feature_dim=64,**kwargs):
        super().__init__()
        ...
        if self.hyper_ad:
            self.raw_image_hyper_ad = RawImageHyperADExtractor(
                ...
                feature_dim=self.embed_dim,  # 输出维度与输入相同
                reduction_ratio=self.reduction_ratio
            )
            
            # 添加hyper_projection用于生成FiLM风格的特征调制参数
            self.hyper_projection = nn.Sequential(
                nn.Linear(self.embed_dim, self.num_features * 2),
        ...
        if self.EDL:
            self.head = MultiBranchHead(
                in_dims=(self.embed_dim, self.num_features, self.num_features),
                proj_dim=256,
                num_classes=self.num_classes,
                mode="edl",  # 输出 evidence
                p_drop=0.1  # 可调，过拟合时加一点
            )
        else:
            self.head = MultiBranchHead(
                in_dims=(self.embed_dim, self.num_features, self.num_features),
    ...
    def forward_backbone(self, x):
        ...
        if self.hyper_ad:
            ...
            R_rgb = torch.zeros(R_fs.size(0), self.embed_dim, device=x.device, dtype=x.dtype)
            branches = (R_rgb, R_pc, R_fs)
